...
function BasicTable()
{
	...
	require 'config/database.php';
	$ida=$_GET['ida'];
	$req=$db->prepare('SELECT * FROM service INNER JOIN login
              ON service.idc=login.id WHERE ida = ? ');
	...
	$this->Image('bama.jpg',8,1,190);
		...
		$this->Cell(40,6,'Heure',1,0,'C');
		$this->Cell(60,6,'Montant en $',1,0,'C');
	...
	while ($row = $req->fetch(PDO::FETCH_OBJ)) {
		...
		$this->Cell(40,8,$row->username,1,0,'C');
		$this->Cell(40,8,$row->datej,1,0,'C');
		$this->Cell(40,8,$row->heure,1,0,'C');
		$this->Cell(60,8,$row->fin,1,0,'C');
		$this->Ln();
		...
		$this->Cell(160,18,$row->detail,1,0,'C');
		$this->Ln();
